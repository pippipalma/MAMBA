function test3(clin_table, path_proc, folder_out)
...
delta = zeros(size(CCS.mfix));

delta(CCS.mfix) = VBmodel(T, CCS.config.outcome, conf, 'mfix', CCS.mfix, ...
    'vars', CCS.config.vars, 'outs', 'delta');
...
mkdir(folder_out)
save(fullfile(folder_out, ['out_' conf]), 'p', 'CCS', 'delta')